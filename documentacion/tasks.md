...
**Plan E2E cross-repo activo.** **M0→M4 en `master` (`829e0e7`)** (PRs #2 + #3). El loop E2E de M4
(demoCompose C4/C5/**C10** + agente A3) cerró el 2026-09-08: `plan-e2e.md §7.1/§7.4` fila M4 →
`coord-test-ready = y`. **Próximo backend: M5 · B4 (Stripe).** Contrato canónico en
`demoCompose/docs/plan-e2e.md` §6; `documentacion/openapi.json` lo refleja.
...
Server de prueba: backend `node dist/main.js` en `:3000` (código de `master`, PID en
`%TEMP%\mb-m1.pid`), contra el RDS pre-prod. Log en `%CLAUDE_JOB_DIR%\tmp\mb-3000.log` mientras
viva esa sesión de background.
...
- **M6 · B7 · search por tokens** — `feat/e2e-m6-i18n-seed @ 9e26d54` (cortada de `master 829e0e7`),
  pusheada, sin merge. `GET /products/search` tokeniza `q`; cada palabra tiene que estar en `name`
  OR `description` (antes: frase completa contra `name`). `products.service.spec.ts` 4/4 + smoke
  RDS. **Falta la parte de datos** (traducir `categories.name` / `products.description` /
  display-names de `Color` a es-419 + migración + `feed.json`) — toca datos del RDS pre-prod,
  espera decisión del usuario. Sin eso "remera negra" aún no matchea.
...
- **M4 · B3 + B5 · Ciclo de vida + eventos.** ✅ en `master` (`9dd1b23` vía PR #2; `c056b8e` Capa 2;
...
- **M6 · B7 · i18n del seed (acotado).** 🚧 search por tokens hecho en rama (ver "doing"). Falta la
  parte de datos: `categories.name` / `store` / `description` / display-names de `Color` → es-419
  (migración + `feed.json`). Paralelo desde M0.
